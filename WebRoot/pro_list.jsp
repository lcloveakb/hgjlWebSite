...
<link rel="stylesheet" type="text/css" href="lib/css/admin/list_style.css">
  ...
  <body>
    <select name="cats" id="cats">
    	...
    	<option value="0" <c:if test="${category==0 }">selected="selected"</c:if>>读写器</option>
    	<option value="1" <c:if test="${category==1 }">selected="selected"</c:if>>标签</option>
    	<option value="2" <c:if test="${category==2 }">selected="selected"</c:if>>其它</option>
    </select>
    
    <table class="data_list" cellpadding="10" align="center">
    	<tr><td>产品名称</td><td colspan="3">操作</td></tr>
    	<c:forEach items="${proList }" var="p">
    	<tr><td>${p.title }</td><td><a href="product/productAction_toUpdate.action?id=${p.id }">修改</a></td>
    		<td><a id="${p.id }" class="del_href">删除</a></td>
    		<td><a id="${p.parentId }" class="top_href">置顶</a>
    		</tr>
    </c:forEach>
    </table>
			...
			<p class="mod_pagenav_main">
				<c:choose>
					<c:when test="${pageno>1}">
						<a class="c_tx " href="product/productAction_listInCategoryBackend.action?pageno=${pageno-1}">上一页</a>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					<c:when test="${pageno<pagecount}">
						<a class="c_tx " href="product/productAction_listInCategoryBackend.action?pageno=${pageno+1}">下一页</a>
					</c:when>
					<c:otherwise>
			...
			<script type="text/javascript">
				function go_to() {
					...
					if (!isNaN(no)) {
						...
						window.location.href = 'product/productAction_listInCategoryBackend.action?pageno='
								+ no + '&pagesize=' + size;
					} else {
						alert("只允许输入数字");
						return false;
		...
		$(function() {
			$("#cats").change(	//类别切换
				function() {
					...
					window.location.href = "product/productAction_listInCategoryBackend.action?category="+ categoryid;
			...
			$(".del_href").each(function(){
				...
				a.bind("click",function() {
					if(window.confirm("确认删除该产品？")) {
						$.ajax({
							url : "product/productAction_delete.action",
							type : "post",
							data : {
								"id" : a.attr("id")
							},
